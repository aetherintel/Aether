# aether_lib/queue_client/queue_client.py
"""
...
class QueueClient:
    def __init__(self):
        ...
        self.queues = {
            'telegram':       Queue('telegram-jobs',       connection=Redis(host=redis_host, port=redis_port, db=0)),
            'translation':    Queue('translation-jobs',    connection=Redis(host=redis_host, port=redis_port, db=1)),
            'image':          Queue('image-jobs',          connection=Redis(host=redis_host, port=redis_port, db=2)),
            'audio':          Queue('audio-jobs',          connection=Redis(host=redis_host, port=redis_port, db=3)),
            'emotion':        Queue('emotion-jobs',        connection=Redis(host=redis_host, port=redis_port, db=4)),
            'classification': Queue('classification-jobs', connection=Redis(host=redis_host, port=redis_port, db=5)),
            'geolocation':    Queue('geolocation-jobs',    connection=Redis(host=redis_host, port=redis_port, db=6)),
        }
