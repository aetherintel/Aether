# workers/classification_worker/modal_worker.py
"""
Thin Modal-forwarding classification worker.
...
MODAL_CLASSIFICATION_URL = os.getenv("MODAL_CLASSIFICATION_URL", "")
...
def classify_post_job(
    ...
    try:
        ...
        logger.error(f"❌ Modal classification failed for {message_id}: {e}")
        raise

    classifications = resp.json().get("classifications", [])
    logger.info(f"✅ Modal classification done for {message_id}: {len(classifications)} labels")

    asyncio.run(_write_neo4j(message_id, classifications, owner_id))
...
async def _write_neo4j(message_id: str, classifications: list, owner_id: str):
    ...
    try:
        await store_classifications_in_neo4j(driver, message_id, classifications, owner_id)
    finally:
        await driver.close()
