...
async def _save_message(tx, channel_id, username, message, sender, media_path):
    ...
    await tx.run(
        ...
        fname=getattr(sender, "first_name", None) if sender else None,
        lname=getattr(sender, "last_name", None) if sender else None,
        uusername=getattr(sender, "username", None) if sender else None,
