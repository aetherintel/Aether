...
const TgChannelsCheckboxList: React.FC<TgChannelsCheckboxListProps> = ({
  structuredChannels,
  ...
  const handleChildChange = (childId: string, parentId?: string) => {
    setSelectedTgChannelIds((current) => {
      ...
      const updated = exists ? current.filter((id) => id !== childId) : [...current, childId];

      // Optional: Update parent if all children are unchecked
      if (parentId) {
        const allChildrenUnchecked = Object.values(
          structuredChannelsById[parentId]?.recommended || {}
  ...
  return (
    <Stack>
      {structuredChannels.map(([, groupValue]) => {
        ...
        const indeterminate =
          recommendedIds.some((id) => selectedTgChannelIds.includes(id)) && !allChecked;
